...
// 1. Base de datos de comidas ampliada con opciones altas en carbohidratos (TU CÓDIGO ORIGINAL)
    ...
    Swal.fire({
        ...
        html: `
            ...
            <div class="fitia-container">
                ...
                <div class="fitia-section">
                    <div class="fitia-header">
                        <div><h3 class="fitia-title">Carbohidratos</h3><span class="fitia-subtitle">Elige tus favoritos</span></div>
                    ...
                    <div class="fitia-grid" id="grid-carb">
                        ...
                        <input type="checkbox" id="c_frijol" class="fitia-checkbox pref-check" value="frijol" checked>
                        <label for="c_frijol" class="fitia-label">🫘 Frijoles</label>
                        <input type="checkbox" id="c_lenteja" class="fitia-checkbox pref-check" value="lenteja">
                        <label for="c_lenteja" class="fitia-label">🧆 Lentejas</label>
                        <input type="checkbox" id="c_garbanzo" class="fitia-checkbox pref-check" value="garbanzo">
                ...
                <div class="fitia-section">
                    ...
                    <div class="fitia-grid" id="grid-grasas">
                        ...
                        <input type="checkbox" id="g_nueces" class="fitia-checkbox pref-check" value="nuez">
                        <label for="g_nueces" class="fitia-label">🥜 Nueces / Maní</label>
                        <input type="checkbox" id="g_lacteos" class="fitia-checkbox pref-check" value="leche">
                        <label for="g_lacteos" class="fitia-label">🥛 Leche / Yogurt</label>
                        <input type="checkbox" id="g_queso" class="fitia-checkbox pref-check" value="queso">
        ...
        preConfirm: () => {
            ...
            const preferencias = Array.from(checks).map(cb => cb.value.toLowerCase());
        ...
        if (result.isConfirmed) {
            ...
            // Filtramos conectándonos directamente a la base de datos de RECETAS
            let comidasFiltradas = recetas.filter(receta => {
                // Unimos el título y los ingredientes para buscar coincidencias de texto
                const textoReceta = (receta.titulo + " " + receta.ingredientes.join(" ")).toLowerCase();
                const coincidePreferencia = preferencias.some(pref => textoReceta.includes(pref));
                const tieneExcluido = excluidos.length > 0 && excluidos.some(ex => textoReceta.includes(ex));
                
                return coincidePreferencia && !tieneExcluido;
            });

            // Si el filtro es muy estricto y quedan pocas recetas, mostramos todas para no romper la app
            if (comidasFiltradas.length < 5) {
                comidasFiltradas = recetas;
            }
            ...
            for (let i = 0; i < 5; i++) {
                ...
                let comida = comidasMezcladas[0]; 
                
                // Creamos las tarjetas clickeables con imágenes
                diasHTML += `
                    <div class="plan-dia-card" onclick="abrirReceta(${comida.id}); Swal.close();">
                        <img src="${comida.imagen}" class="plan-dia-img">
                        <div class="plan-dia-info">
                            <span class="plan-dia-dia">${diasSemana[i]}</span>
                            <h6>${comida.titulo}</h6>
                            <small>${comida.nutricion.calorias} | ${comida.nutricion.proteina} Proteína</small>
                        </div>
                        <i class="fa-solid fa-chevron-right" style="color: #ccc; padding-right: 5px;"></i>
                    </div>
                `;
            }
            ...
            Swal.fire({
                title: `Plan: ${objetivo}`,
                html: `
                    <div class="recom-modal-content" style="max-height: 60vh; overflow-y: auto; padding-right: 5px; text-align: left;">
                        <p style="font-size: 0.85rem; color: #636e72; margin-bottom: 15px;">
                            <i class="fa-solid fa-circle-info" style="color: #3498db;"></i> Haz clic en cualquier platillo para ver los ingredientes y su modo de preparación.
                        </p>
                        ${diasHTML}
                        <h5 style="margin-top: 20px;"><i class="fa-solid fa-dumbbell"></i> Enfoque</h5>
                        <p style="font-size: 0.85rem; color: #636e72;">Tus porciones deben estar alineadas a tu meta de <b>${objetivo}</b>. Adapta los ingredientes de la receta según tus macros diarios.</p>
                    </div>
                `,
...
const recetas = [
    {
        ...
        nutricion: { calorias: '280 cal', proteina: '35g', carbos: '2g', grasas: '6g' }
    },
    {
        id: 1,
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (cerrarBtn) { cerrarBtn.addEventListener('click', cerrarReceta); }
    if (modal) {
